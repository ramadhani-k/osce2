...
export default function RubricsPage() {
  ...
  const fetchRubrics = async () => { setLoading(true); const { data } = await supabase.from('rubrics').select('*').order('title', { ascending: true }); if (data) setRubrics(data); setLoading(false) }
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (editingRubric) { await supabase.from('rubrics').update(formData).eq('id', editingRubric.id) } else { await supabase.from('rubrics').insert([formData]) }
    ...
    <div className="space-y-6">
      ...
      <Card><CardContent className="p-0">
        {loading ? <div className="p-8 flex justify-center"><Loader2 className="animate-spin" /></div> : (
          <Table><TableHeader><TableRow><TableHead>Judul Rubrik</TableHead><TableHead className="text-right">Aksi</TableHead></TableRow></TableHeader>
          <TableBody>{rubrics.map(rubric => (<TableRow key={rubric.id}><TableCell><Link href={`/admin/rubrics/${rubric.id}`} className="flex items-center gap-2 text-primary hover:underline"><FileText size={16} />{rubric.title}</Link></TableCell>
